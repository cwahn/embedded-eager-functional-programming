...
namespace efp {
    ...
    struct Composed<F, Fs...> {
        ...
        Composed(const F& f, const Fs&... fs)
            : f(f), fs(Composed<Fs...>(fs...)) {}
    ...
    auto take(N n, const As& as) -> TakeReturn<N, As, true> {
        return TakeReturn<N, As, true>(
            data(as),
            min(static_cast<size_t>(n), static_cast<size_t>(length(as)))); // Safeguarding against n > length(as)
    }
    ...
    using IntercalateReturn = Conditional<
        ...
        Conditional<
            ...
            ArrVec<
                Element<Element<Ass>>,
                CtCapacity<Ass>::value*(CtCapacity<Element<Ass>>::value + CtCapacity<As>::value) - CtCapacity<As>::value>,
            Vector<Element<Element<Ass>>>>>;
